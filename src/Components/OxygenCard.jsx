...
const OxygenCard =(props)=>{
    ...
    return(
        ...
        <div className="row shadow w-100 m-auto rounded py-4 bg-card position-relative" >
            ...
            <div className="m-p-custom" style={{backgroundColor:"#FE9574"}}>{phone}</div>
            )}
            </div>
            <div className="col-8 mb-3 w-100 rounded-corner d-flex align-items-center" style={{backgroundColor:'#BFE9FB'}}>
                <p className="text-left m-1">
                    <h4>{props.item?.location.locationName}</h4>
                    <p className="text-center">{props.item?.location.address}</p>
                    <span><span className="font-weight-bold">Stock: </span>{props.item?.stock>0?props.item?.stock:(props.item?.stock==0?"Available":"Not Available")}<br/></span><br/>
                </p>
